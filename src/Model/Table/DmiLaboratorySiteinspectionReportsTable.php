<?php
	...
	class DmiLaboratorySiteinspectionReportsTable extends Table{
		
		var $name = "DmiLaboratorySiteinspectionReports";
		...
		public function sectionFormDetails($customer_id){
			...
			$DmiLaboratoryChemistsDetails = TableRegistry::getTableLocator()->get('DmiLaboratoryChemistsDetails');
	   ...
	   public function saveFormDetails($customer_id,$forms_data){
					...
					$htmlencoded_inspection_date = htmlentities($forms_data['inspection_date'], ENT_QUOTES);	
				...
				//check if new file is selected	while reply if not save file path from db
					...
					else{ $created = $CustomersController->Customfunctions->changeDateFormat($report_details[0]['created']); }
					
					$htmlencoded_inspection_date = $CustomersController->Customfunctions->changeDateFormat($htmlencoded_inspection_date);
			...
			public function saveReferredBackComment($customer_id,$report_details,$reffered_back_comment,$rb_comment_ul){
					...
					$formSavedEntity = $this->newEntity(array(
						...
						'inspection_date'=>$report_details['inspection_date'],
