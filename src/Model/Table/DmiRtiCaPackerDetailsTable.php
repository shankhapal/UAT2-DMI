...
class DmiRtiCaPackerDetailsTable extends Table{
	...
	public function sectionFormDetails($customer_id)
	{
		...
		if($latest_id != null){
			...
			'io_reply_date' =>"", 'form_status' =>"",'referred_back_by_email' =>"", 'referred_back_by_once' =>"", 'current_level' =>"", 'delete_ro_referred_back' =>"",'analytical_result_docs'=>"",'shortcomings_noticed_docs'=>'','signnature_of_packer_docs'=>'','signnature_of_inspecting_officer_docs'=>''); 
	...
	public function saveFormDetails($customer_id,$forms_data){
		...
		if(!empty($section_form_details[0]['id'])){
		    ...
		    $date_last_inspection  =  $forms_data['date_last_inspection'];
				...
				// $grading_lab  = $forms_data['grading_lab'];
      ...
      // $replica_account_correct = $forms_data['replica_account_correct'];
			...
			// $htmlencoded_name_Inspecting_officer = htmlentities($forms_data['name_of_inspecting_officer'], ENT_QUOTES);
			// $Designation_inspecting_officer =  $forms_data['designation_inspecting_officer'];
			// $name_of_packer = $forms_data['name_packer_representative'];
