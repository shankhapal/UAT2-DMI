...
class DmiECodeAllotmentDetailsTable extends Table{
	
	var $name = "DmiECodeAllotmentDetails";
	var $useTable = 'dmi_e_code_allotment_details';
		...
		if($dataValidatation == 1 ){
			...
			foreach($forms_data['commodity'] as $key=>$val){
				...
				$dataValues[] = array(
					...
					'alloted_rep_from'=>$cur_rep_no_from,//in e-code appl, same no. will be alloted for all packets
					'alloted_rep_to'=>$cur_rep_no_upto,//in e-code appl, same no. will be alloted for all packets
					'created'=>$date,
					'modified'=>$date
