...
$(document).ready(function () {
        ...
        if ($.trim(response) == "no") {
          $.alert({
            ...
            content:
              ...
              trans_id +
              '" is already used. Please verify and enter again.',
            type: "red",
            typeAnimated: true,
            buttons: {
              tryAgain: {
  ...
  // Added Validation of Entering Amount is not more than 100 corors
  // by shankhpal shende on 17/07/2023
  $("#payment_amount").on("input", function () {
    let value = $(this).val();

    let maxLength = 1000000000; // 100 crores
    var numericValue = value.replace(/\D/g, "");
    // Check if the numeric value exceeds the maximum
    if (parseInt(numericValue) > maxLength) {
  ...
  if ($('input[name="bharatkosh_payment_done"]:checked').val() == "yes") {
    if (input_payment_amount == "") {
      ...
      $("#payment_amount").click(function () {
        $("#error_payment_amount").hide().text;
      });
      value_return = "false";
    } else {
      if (input_payment_amount != actual_payment_amount) {
    ...
    if (input_payment_transaction_id == "") {
      $("#error_payment_transaction_id")
        .show()
        .text("Please enter payment transction id");
      $("#error_payment_transaction_id").css({
        color: "red",
        "font-size": "14px",
        "font-weight": "500",
        "text-align": "right",
      });
      $("#payment_transaction_id").click(function () {
        $("#error_payment_transaction_id").hide().text;
      });
      value_return = "false";
    }

    if (input_payment_trasaction_date == "") {
      $("#error_payment_trasaction_date")
        .show()
        .text("Please select payment transction date");
      $("#error_payment_trasaction_date").css({
        color: "red",
        "font-size": "14px",
        "font-weight": "500",
        "text-align": "right",
      });
      $("#payment_trasaction_date").click(function () {
        $("#error_payment_trasaction_date").hide().text;
      });
      value_return = "false";
    }
    if ($("#payment_receipt_document_value").text() == "") {
      if (input_payment_receipt_document == "") {
        $("#error_payment_receipt_document")
          .show()
          .text("Please upload payment receipt");
        $("#error_payment_receipt_document").css({
          color: "red",
          "font-size": "14px",
          "font-weight": "500",
          "text-align": "right",
        });
        $("#payment_receipt_document").click(function () {
          $("#error_payment_receipt_document").hide().text;
        });
        value_return = "false";
      }
    }
