	...
	*/
	...
	function appl_common_list_js_call(){
		...
		if(current_level_script_id=='level_2' || current_level_script_id=='level_3' || current_level_script_id=='level_4' || current_level_script_id=='pao'){
			...
			$("#common_app_list_table").on("click","#allocate-routine-inspection" + p,function () {
	...
	function allocation_common_tabs_js_call(){
		...
		$("#for_routine_inspection_count").text(countarray['routine_inspection_allocation_tab']);	// For Routine Inspection (RTI) added by shankhpal shende on 06/12/2022
			...
			if (appl_type != "" && io_user_id != null && customer_id != "" && customer_id != "") {
				...
				$.ajax({
					...
					success: function (data) {
						...
						$(".loadermsg").hide();
						$("#inspection_alloction_Modal").hide();
						alert("The Application " +customer_id +" is successfully allocated for Routine Inspection to IO user.");
						//to reload list after allocation
